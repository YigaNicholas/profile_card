...
function Sidebar({ isAuthenticated, setIsAuthenticated }) {
  ...
  return (
    <>
      {/* Mobile menu toogle button/ harmbuger menu */}
      {/*toogle isOpen between true and false*/}
      <button 
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-64 bg-white shadow-lg transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out`}>
        <div className="flex items-center justify-center h-16 px-4 bg-yellow-600">
          <span className="text-white font-bold text-xl">Dashboard</span>
        </div>
        <nav className="mt-5 px-2 space-y-1">
          <Link to="/" className="flex items-center px-2 py-3 text-base font-medium text-white bg-blue-700 rounded-lg group">
            <svg className="mr-4 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            Dashboard
          </Link>
          <Link to="/team" className="flex items-center px-2 py-3 text-base font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-lg group">
            <svg className="mr-4 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            ...
            className="w-full flex items-center px-2 py-3 text-base font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-lg group"
          >
            <svg className="mr-4 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
